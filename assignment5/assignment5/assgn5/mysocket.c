...
Received_Message rmsg;

typedef struct _sent_msg{
    message Sent_Message[LIMIT_LEN];
    int _start;
    int _end;
} Sent_Message;
...
void *__recv_handler(void * sock_in) {
    ...
    while ( 1 ) {
        ...
        int push_idx = rmsg._end;
        
        rmsg.Received_Message[ push_idx ].msg_len = recvd_msg.msg_len;
        strncpy(
            rmsg.Received_Message[ push_idx ].content
            , recvd_msg.content
            , recvd_msg.msg_len
        );

        __recv_cnt ++;
        rmsg._end = (rmsg._end + 1) % LIMIT_LEN;
...
int my_recv(int sockfd
    ...
    message msg = rmsg.Received_Message[ rmsg._start + 1];
    int final_len = (len < msg.msg_len) ? len: msg.msg_len;
    ...
    if ( flags & MSG_PEEK )
        ...
        rmsg._start = ((rmsg._start + 2) % LIMIT_LEN) - 1;
    }
...
void *__send_handler(void *sock_in) {
    ...
    while ( 1 ) {
        ...
        __send_cnt --;
        int pop_idx = smsg._start + 1;

        send_msg.msg_len = smsg.Sent_Message[ pop_idx ].msg_len;
        strncpy(
            send_msg.content
            , smsg.Sent_Message[ pop_idx ].content
            , send_msg.msg_len
            );
        send_msg.flags = smsg.Sent_Message[ pop_idx ].flags;

        smsg._start = (smsg._start + 2) % LIMIT_LEN - 1;
        ...
        printf("[__send_handler] calling __format_send\n");
        ...
        printf("[__send_handler] done with __format_send: %s\n", send_msg.content);
    }
...
int my_send(int sockfd
    ...
    int push_idx = smsg._end;

    smsg.Sent_Message[ push_idx ].msg_len = len;
    memcpy(
        smsg.Sent_Message[ push_idx ].content
        , buf
        , len + 1
        );
    smsg.Sent_Message[ push_idx ].flags = flags;

    // setting the shared variables
    __send_cnt ++;

    smsg._end = (smsg._end + 1) % LIMIT_LEN;
