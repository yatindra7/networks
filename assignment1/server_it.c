...
double expression_value(char *expr, char **err) {
    ...
    while ((tok = expr[i]) != '\0') {
        ...
        if (isspace(tok)) { /* do nothing */}
        ...
        else if (tok == '(') push_char(&stck_chr, tok);
            ...
            char err_msg[40] = "[ERROR] UNRECOGNISABLE CHARACTER\n";
            *err = (char *) malloc(strlen(err_msg));
            strcpy(*err, err_msg);
            return -1;
        }
        i ++;
...
int main () {
    ...
    while (1) {
        ...
        int flag;
        char *expr = variable_length_recv(conn_sock_fd, &flag), *err_expr;
        ...
        double ans_val = expression_value(expr, &err_expr);

        if (err_expr != NULL) {
            printf("[ERROR] %s", err_expr);
            send(conn_sock_fd, err_expr, strlen(err_expr) + 1, 0);
        ...
        printf("[LOG] ANSWER VALUE TO BE SENT: %lf\n", ans_val);
