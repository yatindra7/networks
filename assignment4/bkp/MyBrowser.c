...
void handle_request(char *cmd) {
    ...
    char *url, *path, *req, *file_pt;         /*!< variables to store results 
                                    post parsing*/
    ...
    handle_connection(url, path, req, file_pt, port);

    free(url); free(path); free(req); free(file_pt);
}
...
void parse_cmd(
    ...
    *url = (char *) malloc ( 
    ...
    if ( strcasecmp(*req, "PUT") == 0 ) {

        char *file_to_put = strtok(NULL, " ");
        *file_pt = (char *) malloc ( ( strlen(file_to_put) + 1 ) * sizeof (char) );
        strcpy(*file_pt, file_to_put);
    } else {
        *file_pt = NULL;
    }
    return;
